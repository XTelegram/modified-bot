...
def get_readable_message():
    with download_dict_lock:
        ...
        for index, download in enumerate(list(download_dict.values())[COUNT:], start=1):
            ...
            msg += f"\n<b>❓  Status:</b> <i><a href='{download.message.link}'>{download.status()}</a></i>"
            if download.status() not in [MirrorStatus.STATUS_SEEDING, MirrorStatus.STATUS_SPLITTING]:
                if config_dict['EMOJI_THEME']:
                    msg += f"\n<b>💥  </b>{get_progress_bar_string(download)} {download.progress()}"
                    msg += f"\n<b>📦  Ukuran:</b> {get_readable_file_size(download.processed_bytes())} / {download.size()}"
                ...
                if config_dict['EMOJI_THEME']:
                    ...
                    msg += f"\n<b>⚙️  Engine:</b> <code>qBittorrent</code>"
        ...
        buttons.sbutton("📊 Statistik", str(THREE))
        buttons.sbutton("❌ Tutup", "status close")
