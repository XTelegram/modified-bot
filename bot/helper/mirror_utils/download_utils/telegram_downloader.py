                ...
                if config_dict['STOP_DUPLICATE'] and not self.__listener.isLeech:
                    ...
                    if smsg:
                        if config_dict['TELEGRAPH_STYLE']:
                            return sendMarkup("⚠️ File/Folder sudah ada 🤗", self.__listener.bot, self.__listener.message, button)
                        else:
                            return sendFile(self.__listener.bot, self.__listener.message, f_name, f"⚠️ File/Folder sudah ada 🤗\n\n{smsg}")
